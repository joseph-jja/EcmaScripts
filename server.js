...
for ( let i = 2, end = process.argv.length; i < end; i++ ) {
    if ( process.argv[ i ].indexOf( RUN_DIR ) > -1 ) {
        ...
        port = parseInt( process.argv[ i ].substring( idx ), 10 );
    }
}

const statfile = util.promisify( fs.stat );

const initalFile = `${baseDir}/coverage/report-html/index.html`;

async function listDir( dir, response, referer = '' ) {

    const cdir = dir.replace( /\/\//g, '/' );
    const fullpath = path.resolve( runDir, cdir );
    const isDir = await statfile( fullpath ).catch( _err => {
        return null;
    } );
    ...
    if ( isDir?.isDirectory() ) {
        ...
        let ltype = 'text/html';
        const results = await viewFile( fullpath, runDir ).catch( _err => {
...
const server = httpOrhttps.createServer( options, ( request, response ) => {
    ...
    if ( searchParams && searchParams.saveFile ) {
        ...
        if ( !fullpath.startsWith( runDir ) ) {
            ...
            console.error( 'Path error: ', fullpath.startsWith( runDir ) );
        }
        statfile( fullpath )
            .then( _stats => {
                let res = [];
