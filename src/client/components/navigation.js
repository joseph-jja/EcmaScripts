...
allLinks = [ ["index.html", "Main Page","The index page"],
...
{ 
	...
	function getCurrentKey() {
		...
		if ( path.indexOf("fish_pictures") ) { url += "_fish"; }
	...
	function addMetrics() {
		var key, prop, mix = [], i, len, data = "", store, sdata, render, xtitle; 
		...
		mix.push({ 'name': "Dom Content Load Start", 'value' : prop['domContentLoadStart'] });
		mix.push({ 'name': "Dom Content Load Event Length", 'value': prop['domContentLoadTime'] });
		mix.push({ 'name': "Dom OnLoad Start", 'value': prop['domLoadStart'] });
		mix.push({ 'name': "Dom OnLoad Event Time", 'value': prop['domLoadTime'] });
		mix.push({ 'name': "Your Network Latency", 'value': prop['networkLatency'] });
		mix.push({ 'name': "Total Page Load Time", 'value': prop['pageLoadTime'] });		
		...
		if ( key === 'performance' ) {
			 function render() {
				var x, xlen, xkey, xdata, xresult = "", xjson, xitem, y, xdlen, element, ytitle;
				xlen = store.length;
				for ( x = 0; x < xlen; x+=1 ) { 
			    	xkey = store.key(x);
...
function showMenu() { 
	var dom = WebBrowser.dom, css =  WebBrowser.css, 
